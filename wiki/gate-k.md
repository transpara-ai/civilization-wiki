...
sources:
  ...
  - raw/transpara/dark-factory/v4.0/implementation/epics/epic-01-interim-loop-hardening/07-gate-k-pre-go-live-waiver-v4.0.md  # Gate K pre-go-live waiver and closeout disposition, transpara-ai/docs#138; private merge evidence rechecked internally on 2026-06-17
confidence:
  sources: primary
  claims: grounded
---
...
**The first v4.0 program gate.** Gate K is the evidence predicate that the interim development loop has been hardened — meaning the controls that govern the ChatGPT/Codex/Claude build loop have been made fail-closed on `transpara-ai/docs`. It is defined in `DF-V4.0-INTEGRATION-ARC` (00-integration-arc-v4.0.md) as "Event 1 (Gate K): Interim Development Loop Tier-0 Hardening." As of 2026-06-17, the Event-1 authorization has been **granted** (decision *Notify*, recorded on `transpara-ai/docs#132`) and Gate K is **closed for pre-live sequencing by human waiver** (`transpara-ai/docs#138`; private merge evidence rechecked internally on 2026-06-17). That closure is not vendor-posture evidence, not production go-live clearance, and not an autonomy increase. It continues the [[gates|Gates A–J]] lettering, adding two v4.0 program gates (K and L) without altering any v3.9 gate.
...
On 2026-06-17, internal docs#138 merged the explicit pre-live waiver; private merge evidence was rechecked internally during this PR. The current state is:
