...
The orientation doc is explicit that it "is not the current-state register" — the v3.9 state-of-the-system proof and as-built tracker remain authoritative for gate status. As of the orientation draft, the posture rule is: "v3.9 records bounded completions through Gate J and Epic 10, Epic 11 is selected only for authorization review, and v4.0 Gate K / Gate L remain proposal-only future gates." [DF-V3.9-STATE:L31-L35] [DF-V3.9-TRACKER:L176-L196] [DF-V4.0-ARC:L115-L149] The autonomy progression is incremental and conditional: per the tenth invariant, autonomy expands only when audit and rollback improve with it.
