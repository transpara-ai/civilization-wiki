...
First-party synthesis: `Dark Factory - Motive, Goal, Approach.md` and its embedded post corpus — the definition of **Blind** ("What you don't know you don't know. The most dangerous state") in the Layer 0 instrumentation group (`InstrumentationSpec · CoverageCheck · Gap · Blind`), and the record that a [[44-primitives|hive of autonomous agents]] independently derived `Blind` and related self-ignorance concepts as necessary for operating in an untrusted world. Used to corroborate the essay's use of `Blind`, not to extend it.
