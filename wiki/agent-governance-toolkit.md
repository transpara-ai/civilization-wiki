...
This article is about **our investigation and decision**, not a mirror of the upstream's documentation. The upstream is public, lovyou-ai-adjacent code; per org rule it is cited here only as context and is never re-published.
...
- **Upstream (context only, not re-published)** — `github.com/microsoft/agent-governance-toolkit`. Cited solely to explain *why we forked it to read it*; the wiki subject is our investigation and our decision. Per org rule the upstream is public, lovyou-ai-adjacent code — never re-published, never pushed, never PR'd.
