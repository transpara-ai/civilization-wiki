...
This article is about **our investigation and decision**, not a mirror of the upstream's documentation. The NousResearch repos are public, lovyou-ai-adjacent code; they are cited here only as context for *why we forked them to read them*, never re-published.
...
- **Upstream cited, never re-published.** Per org rule, `NousResearch/hermes-agent` (and the two sibling repos) are public, lovyou-ai-adjacent code. Their descriptions ("The agent that grows with you"; the DSPy/GEPA self-evolution line) and MIT licensing are corroborated live as context for *why we forked them to read them*, not as content to mirror. The wiki subject is our investigation and our decision.
